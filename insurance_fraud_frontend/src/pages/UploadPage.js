    ...
    <div className="grid">
      ...
      <div className="card" style={{ gridColumn: "span 7" }}>
        ...
        {fileMeta ? (
          ...
          <div className="helpText" style={{ marginTop: 10 }}>
            Tip: include columns like <span className="mono">claim_id</span>, <span className="mono">policy_id</span>, <span className="mono">claim_amount</span>, <span className="mono">claimant_name</span>, <span className="mono">date_of_loss</span>.
          </div>
        )}
              ...
              {preview.rows.slice(0, 5).map((row, idx) => (
                <tr key={idx}>
                  {(preview.fields || Object.keys(row || {})).slice(0, 4).map((f) => (
                    <td key={f} className="mono">{String(row?.[f] ?? "—").slice(0, 26)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
